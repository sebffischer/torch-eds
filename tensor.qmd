...
While similar to R matrices, tensors offer enhanced capabilities that make them particularly suited for modern computational tasks.
...
Like for R arrays, each tensor has a shape and a dimension:
...
if (cuda_is_available()) { # Check if cuda is available
  ...
  x <- x$to(device = torch_device("mps"))
} else {
...
* 1-dimensional tensors are converted to R vectors:
  ...
  ```
* `2`-dimensional tensors are converted to R matrices:

  ```{r}
  as.matrix(torch_randn(2, 2)) # as_array() also works
  ````
* `>2`-dimensional tensors are converted to R arrays:
  ```{r}
  as_array(torch_randn(2, 2, 2))
  ```
* `0`-dimensional tensors (scalars) are converted to R vectors with length 1:
   ```{r}
   torch_tensor_scalar(1)$item() # as_array() also works
   ```
...
```{r}
x$reshape(c(3, 4))$shape
```

We can also infer dimensions by setting them to `-1`:

```{r}
x$reshape(c(-1, 3))$shape
```

Not all reshaping operations are valid, however.
