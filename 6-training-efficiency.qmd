...
* Explain difference between preprocessing and augmentation.
* Mention that one should usually use a predefined architecture: Copy it from Python (can link to the torch tutorial to do so)
...
2. Methodological approaches that change how we approach the modeling to get either better results or faster training.
...
`torch` also allows for interop-parallelization, but this is is more advanced and code needs to be written in a specific way.
...
1. Improve implementation of the `dataset` class
2. Parallelizing the data loading process
3. Moving data to the GPU

Both of these approaches will now be discussed.
...
An individual image can e.g. be loaded using the `torchvision::base_loader()` function:
...
In order to use parallel data loading or memory pinning with `mlr3torch`, these parameters can simple be specified in the learner:
...
Some special care needs to be taken when using `torch` (or `mlr3torch`) in order to get a good performance.
...
In `torch`, this can either be done using using TorchScript or by tracing a model.
We will briefly discuss both approaches, but for more infromation see the [torch documentation](https://torch.mlverse.org/docs/articles/torchscript).
...
The alternative to writing TorchScript is to write your module in R and to use use [`jit_trace`](https://torch.mlverse.org/docs/reference/jit_trace_module.html) to compile it.
```{r}
...
An advantage of trace-compilation is that it even allows to jit-compile modules, which is currently not possible with `jit_compile`.
...
Trace-compilation is restrictive because it only records operations appplied to torch tensors and is unaware of R control flow, so you need to be careful when using it.
...
This allows to reduce the memory footprint of the model and to speed up the training process.
...
As we have already seen in one of the previous notebooks, in deep learning one often some part of the data is used for validation purposes.
This allows to monitor the performance of the model on unseen data.
...
mlp_learner = lrn("classif.mlp",
  ...
  callbacks = t_clbk("history"), # history callbacks saves train and validation performance
...
Beyond only tuning the number of epochs, `mlr3`'s internal tuning mechanism also allows to tune the number of epochs internally while using an offline tuning method to optimize other hyperparameters.
...
In `torch`, different versions of batch normalization exists for different dimensions of the input tensor.
...
**Question 1**: Earlier we have learned that `nn_module`s have buffers and parameters, where the latter are learnerd with gradient descent.
...
Which of the following statements (stored in vairiables `equal1` and `equal2`) do you think is true?
...
The second statemnt is false, because the first tensor uses different means and variances for rows 1-2 and 3-4, while the second tensor uses the same mean and variance for all rows.
</details>
:::
...
Recall that the `po("torch_ingress_ltnsr")` is a special `PipeOp` that that marks the input of the neural network.
...
<details>
<summary>Click for answer</summary>
Usually not, as the purpose of data augmentation is not to improve an individual prediction, it will not be applied during test time and hence also not to the validation set.
