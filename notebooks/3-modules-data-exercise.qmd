...
Let's explore how these components work together by building a neural network to classify spiral data. Note that we will only briefly cover optimizers here and dedicate an additional notebook to them.
...
Note that while `nn_module`s act like functions, they also maintain a *state*, most importantly their parameter weights.

It is straightforward to implement a custom `nn_module`, which requires defining two key methods:
...
In addition to parameters and buffers, an `nn_module` operates in either a *train* or *eval* state:
...
The input has \(28 \times 28 = 784\) pixels and the output as well. The weights of the fully connected layer are a \(784 \times 784\) matrix, so the number of parameters is \(784 \times 784 = 614,656\).
...
for(epoch in seq_len(n_epochs)) {
  ...
  coro::loop(for(batch in valid_loader) {
    ...
    valid_acc <- as.array(output$argmax(dim = 2) == y)
