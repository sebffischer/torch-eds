...
In this exercise, we will -- again -- train a simple multi layer perceptron on the *Indian Liver Patient Dataset* (ILPD).
Create a learner that:

1. Uses 2 hidden layers with 100 neurons each
1. Uses a batch size of 128
1. Trains for 200 epochs
1. Uses a validation set of 30% of the data
1. Tracks the train- and validation log-loss during training.
1. Uses trace-jitting to speed up the training process.
1. Uses the history callback to record the train- and validation log-loss during training.

Afterwards, plot the validation log-loss which is accessible via `learner$model$callbacks$history`.
...
* The history callback can be constructed via `t_clbk("history")` and needs to be passed during *construction* of the learner.
...
Enable early stopping to prevent overfitting and re-train the learner (using a patience of 10).
Print the final validation performance of the learner and the early stopped results.
You can consult the [documentation of `LearnerTorch`](https://mlr3torch.mlr-org.com/reference/mlr_learners_torch.html) on how to access these two results (section *Active Bindings*)
...
One thing we have not covered so far, is that the MLP learner we have used so far also uses a dropout layer.
...
Apart from this, the tuning works just like in tutorial 5.
Use 3-fold cross-validation for the tuning and evaluate 10 configurations.
