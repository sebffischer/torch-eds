...
To use autograd, tensors must their `requires_grad` be `TRUE`.
...
We will later use this to change the weights $a$ and $b$ to lead to better predictions.
To write down the gradients, let $u = a \times x$ denote the temporary tensor from the linear relationship.
