...
TODO:
* is `with_no_grad` covered enough?
...
To use autograd, tensors must be created with `requires_grad = TRUE`.
In the context of deep learning, we track the gradients of the weights of a neural network.
The simplest case is a simple linear model, where we have slope $a$ and bias $b$ and a single input $x$.

The forward pass is defined as:
$\hat{y} = a \times x + b$

We could e.g. be interested in how the prediction $\hat{y}$ changes for the given $x$ when we change the weight $a$ or the bias $b$.
Let $u = a \times x$ denote the temporary tensor
...
When you perform operations on tensors with gradient tracking, torch builds a dynamic computational graph.
In the figure below:
...
In order to calculate the gradients $\frac{\partial y}{\partial a}$ and $\frac{\partial y}{\partial b}$ we can simply call these derivative functions, traverse the graphs and multiply the individual derivatives according to the chain rule.
In `torch` this is done by calling `$backward()` on `y`:
 we simply call `$backward()` on `y`.
...
p <- ggplot(data.frame(x = x_r, y = y_r), aes(x = x, y = y)) +
  ...
  geom_abline(intercept = 1.0, slope = 2.5, linewidth = 1,
              color = "blue") +
  theme_minimal() +
  labs(title = "Linear Regression with Gradient Descent",
       x = "X", y = "Y")
p
```
...
We can also visualize the loss landscape with respect to $a$ and $b$:
...
ggplot(grid, aes(x = a, y = b, z = mse)) +
  ...
  annotate("text", x = a_min + 0.3, y = b_min + 0.2,
           label = sprintf("Minimum:\na = %.2f\nb = %.2f", a_min, b_min), color = "red") +
  annotate("text", x = as.numeric(a) + 0.3, y = as.numeric(b) + 0.2,
           label = sprintf("Start:\na = %.2f\nb = %.2f", as.numeric(a), as.numeric(b)), color = "blue") +
  labs(title = "Loss Landscape (MSE)",
       x = "Parameter a",
       y = "Parameter b",
       fill = "MSE Loss") +
  coord_fixed()
...
Note that the gradient only tells us in which direction we have to go, not too far.
...
- Require less computation per update
- Implicitly regularize the optimization
