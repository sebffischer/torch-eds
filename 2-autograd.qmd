...
**Question 1**: Can you guess the value of `requires_grad` for `u`?


<details>
<summary>
Because `x` requires gradient tracking, `u` and `y` will also require gradient tracking.
Because of the chain rule, calculating the gradients of `g(f(x))` requires
