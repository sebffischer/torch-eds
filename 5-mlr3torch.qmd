...
However, as `mlr3torch` is a framework, it is less flexible than `torch` itself, so knowing both is useful.

Another helpful R package that provides many useful functions to train neural networks is [`luz`](https://mlverse.github.io/luz/).


## `mlr3` Recap

Before we dive into `mlr3torch`, we will briefly review the core building blocks of the `mlr3` machine learning framework.
For reference, we recommend the [mlr3 book](https://mlr3book.mlr-org.com/) that explains the `mlr3` framework in more detail.
Furthermore, there exists the [mlr3 website](https://mlr-org.com/) that contains more tutorials and overviews.
...
::: {.callout-tip}
To get the help page for an `mlr3` object, you can e.g. call `tsk_mtcars$help()`.
:::
...
```{r}
tsk_mtcars$data(rows = 1:5, cols = c("mpg", "cyl"))
```
...
4. **Select a Measure**: Define the metric to optimize during tuning.
   ...
   ```{r}
   msr_tune = msr("regr.mse")
   ```

1. **Execute Tuning**: Run the tuning process to find the optimal hyperparameters. Here we also specify our budget of 10 evaluations.
...
```{r}
tune_result$archive$data[order(regr.mse, decreasing = TRUE), ][1, regr.mse]
```
...
To illustrate this, imageine that we take the maximum of 10 random numbers drawn from a normal distribution with mean 0.
...
In order to get a valid performance estimte of the tuning process, we can `resample()` it on the task.
This is called *nested resampling*: the outer resampling is for evaluation and the inner resampling is for tuning.

```{r}
rr = resample(tsk_mtcars, at, rsmp_tune)
rr$aggregate()
```
...
Besides the trained network, the `$model` of the learner e.g. also contains the `$state_dict()` of the optimizer and other information.
...
:::{.callout-tip}
The `$configure()` method of a `Learner` allows to simultanteously set fields and hyperparameters of a learner.
:::
...
```{r}
lrn_mlp$train(tsk_mtcars)
```

After training, the results of the callback can be accessed via the model.

```{r}
head(lrn_mlp$model$callbacks$history$valid)
head(lrn_mlp$model$callbacks$history$train)
```
...
`mlr3torch` also allows to define custom architectures by assembling special `PipeOp`s in a `Graph`.
...
:::{.callout-tip}
In order to avoid having to re-download the dataset every time, you can set the `mlr3torch.cache` option to `TRUE`.
```{r, eval = FALSE}
options(mlr3torch.cache = TRUE)
```
:::
...
This means, it does not necessarily store the data in memory, but only stores the information how to load the data.

```{r}
image_vec = tsk_cifa$data(cols = "image")[[1L]]
head(image_vec)
```

In order to access the data as `torch_tensor`s, we can call the `materialize()` function:

```{r}
image_tensor = materialize(image_vec)[[1L]]
```

To construct the cifar10 task ourselves, we need to first:
...
```{r}
cifar10_original = torchvision::cifar10_dataset(root = "cifar10", train = TRUE, download = TRUE)
...
In order to convert this to a `lazy_tensor` later, the `$.getitem()` method needs to return a named list.
...
The only thing we need to specify are the output shapes of the tensors, which we set to `c(NA, 3, 32, 32)`.
...
We can now train our previously defined graph-learner on the flattened task.

```{r, include = FALSE}
glrn$param_set$set_values(torch_model_classif.epochs = 0)
```

```{r}
glrn$train(tsk_cifar)
```
...
TODO: Also show hyperparameter tuning of deep neural networks here.
  Connect to the previous notebook by tuning the learning rate on a log scale.
